...
class Controller(object):
    def __init__(self, objfun, args, x0, r0, r0_nsamples, r0_tol, work_done, saved_info, xl, xu, npt, rhobeg, rhoend, nf, nx, maxfun,
                 params, scaling_changes, dynamic_accuracy=False):
        self.objfun = objfun
        self.args = args
        self.maxfun = maxfun
        self.dynamic_accuracy = dynamic_accuracy
        self.model = Model(npt, x0, r0, xl, xu, r0_nsamples, r0_tol, saved_info, precondition=params("interpolation.precondition"),
    ...
    def initialise_coordinate_directions(self, number_of_samples, num_directions, params):
        ...
        for k in range(1, num_directions + 1):
            ...
            rvec_list, f_list, num_samples_run, tol, saved_info, exit_info = self.evaluate_objective(x, number_of_samples, params)

            # Handle exit conditions (f < min obj value or maxfun reached)
            if exit_info is not None:
                if num_samples_run > 0:
                    self.model.save_point(x, np.mean(rvec_list[:num_samples_run, :], axis=0), num_samples_run, tol,
                                          x_in_abs_coords=True)
                return exit_info  # return & quit

            # Otherwise, add new results (increments model.npt_so_far)
            self.model.change_point(k, x - self.model.xbase, rvec_list[:num_samples_run, :], tol, saved_info=saved_info,
                                    allow_kopt_update=params("general.accept_any_decrease"))  # expect step, not absolute x
    ...
    def initialise_random_directions(self, number_of_samples, num_directions, params):
        ...
        if params("init.run_in_parallel"):
            ...
            for ndirns in range(num_directions):
                new_point = xopt + dirns[ndirns, :]  # alway base move around best value so far
                x = self.model.as_absolute_coordinates(new_point)
                rvec_list, f_list, num_samples_run, tol, saved_info, exit_info = eval_obj_results[ndirns]
                # Handle exit conditions (f < min obj value or maxfun reached)
                if exit_info is not None:
                    if num_samples_run > 0:
                        self.model.save_point(x, np.mean(rvec_list[:num_samples_run, :], axis=0), num_samples_run, tol,
                ...
                self.model.change_point(1 + ndirns, x - self.model.xbase,
                                        rvec_list[:num_samples_run, :], tol, saved_info=saved_info,
                                        allow_kopt_update=params("general.accept_any_decrease"))  # expect step, not absolute x
        else:
            for ndirns in range(num_directions):
                new_point = xopt + dirns[ndirns, :]  # alway base move around best value so far
                ...
                # Otherwise, add new results (increments model.npt_so_far)
                self.model.change_point(1 + ndirns, x - self.model.xbase, rvec_list[:num_samples_run, :], tol, saved_info=saved_info,
                                        allow_kopt_update=params("general.accept_any_decrease"))  # expect step, not absolute x
    ...
    def add_new_direction_while_growing(self, number_of_samples, params, min_num_steps=0):
        ...
        for j in range(num_steps):
            ...
            self.model.change_point(kmin, xnew, rvec_list[:num_samples_run, :], tol, saved_info=saved_info,
                                    allow_kopt_update=params("general.accept_any_decrease"))  # expect step, not absolute x
    ...
    def geometry_step(self, knew, adelt, number_of_samples, params):
        ...
        # Otherwise, add new results
        self.model.change_point(knew, xnew, rvec_list[:num_samples_run, :], tol, saved_info=saved_info,
                                allow_kopt_update=params("general.accept_any_decrease"))  # expect step, not absolute x
    ...
    def evaluate_objective(self, x, number_of_samples, params, f_tol=None, saved_info=None, fval=None):
        ...
        # In dynamic accuracy version, use accuracy tol (if provided), else delta**2
        if self.dynamic_accuracy:
            max_f = self.model.max_fval() if fval is None else fval
            const = params("general.dynamic_accuracy_delta_factor")
            if f_tol is None:  # accuracy applies to each component of r(x) evaluation
                desired_accuracy = const * self.delta ** 2
            else:
                desired_accuracy = min((sqrt(max_f + f_tol) - sqrt(max_f)) / sqrt(self.m()), const * self.delta ** 2)
            desired_accuracy = max(desired_accuracy, 1e-15)  # floor accuracy requirements
            # logging.debug("With fval=%s, tol=%s" % (str(fval), str(f_tol)))
            # logging.debug("Asking for accuracy %g" % desired_accuracy)
        else:
        ...
        for i in range(number_of_samples):
            ...
            if not incremented_nx:
                self.nx += 1
                incremented_nx = True
            rvec_list[i, :], f_list[i], saved_info, work_done = eval_least_squares_objective(self.objfun, remove_scaling(x, self.scaling_changes), args=self.args, eval_num=self.nf, pt_num=self.nx,
                                            full_x_thresh=params("logging.n_to_print_whole_x_vector"),
                                            check_for_overflow=params("general.check_objfun_for_overflow"),
                                            desired_accuracy=desired_accuracy, saved_info=saved_info)
            num_samples_run += 1
            self.work_done += work_done
        ...
        return rvec_list, f_list, num_samples_run, desired_accuracy if self.dynamic_accuracy else 0.0, saved_info, exit_info
    ...
    def soft_restart(self, number_of_samples, nruns_so_far, params, x_in_abs_coords_to_save=None, rvec_to_save=None,
        ...
        if x_in_abs_coords_to_save is not None:
            ...
            self.model.save_point(x_in_abs_coords_to_save, rvec_to_save, nsamples_to_save, tol_to_save, x_in_abs_coords=True)
        self.model.save_point(self.model.xopt(abs_coordinates=True), self.model.ropt(),
                              self.model.nsamples[self.model.kopt], self.model.tolopt(), x_in_abs_coords=True)
        ...
        if params("restarts.increase_npt") and self.model.npt() < params("restarts.max_npt"):
            ...
            for i in range(num_pts_to_add):
                ...
                # Otherwise, add new results
                self.model.add_new_point(xnew, rvec_list[:num_samples_run, :], tol, saved_info=saved_info,
                                         allow_kopt_update=params("general.accept_any_decrease"))  # expect step, not absolute x
