...
def trsbox(xopt, g, hess, sl, su, delta, use_fortran=USE_FORTRAN):
    if use_fortran:
        return trustregion.solve(g, hess, delta, sl=np.minimum(sl - xopt, -ZERO_THRESH),
                                 su=np.maximum(su - xopt, ZERO_THRESH), verbose_output=True)
...
def trsbox_linear(g, a, b, Delta, use_fortran=USE_FORTRAN):
    if use_fortran:
        return trustregion.solve(g, None, Delta, sl=np.minimum(a, -ZERO_THRESH), su=np.maximum(b, ZERO_THRESH), verbose_output=False)
