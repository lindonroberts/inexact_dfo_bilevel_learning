...
class OptimResults(object):
    def __init__(self, xmin, rmin, fmin, jacmin, tolmin, nf, nx, nruns, exit_flag, exit_msg):
        self.x = xmin
        self.resid = rmin
        self.resid_tol = tolmin
        self.f = fmin
...
def solve_main(objfun, x0, args, xl, xu, npt, rhobeg, rhoend, maxfun, nruns_so_far, nf_so_far, nx_so_far, nsamples, params,
               diagnostic_info, scaling_changes, dynamic_accuracy=False,
               r0_avg_old=None, r0_nsamples_old=None, r0_tol_old=None, default_growing_method_set_by_user=None,
               work_done_old=None, diag_info_save_filestem=None, diag_info_save_freq=1000):
    ...
    if r0_avg_old is None:
        ...
        nf = nf_so_far + 1
        nx = nx_so_far + 1
        tol0 = params("general.dynamic_accuracy_delta_factor") * rhobeg**2 if dynamic_accuracy else 0.0
        r0, f0, saved_info, work_done = eval_least_squares_objective(objfun, remove_scaling(x0, scaling_changes), args=args, eval_num=nf, pt_num=nx,
                                                full_x_thresh=params("logging.n_to_print_whole_x_vector"),
                                              check_for_overflow=params("general.check_objfun_for_overflow"),
                                              desired_accuracy=tol0 if dynamic_accuracy else None,
                                                          saved_info=None)
        m = len(r0)
        ...
        for i in range(1, number_of_samples):  # skip first eval - already did this
            ...
            rvec_list[i, :], f_list[i], saved_info, work_done = eval_least_squares_objective(objfun, remove_scaling(x0, scaling_changes), args=args, eval_num=nf, pt_num=nx,
                                                full_x_thresh=params("logging.n_to_print_whole_x_vector"),
                                                check_for_overflow=params("general.check_objfun_for_overflow"),
                                                desired_accuracy=tol0 if dynamic_accuracy else None,
                                                                                  saved_info=None)
    ...
    control = Controller(objfun, args, x0, r0_avg, num_samples_run, tol0, work_done, saved_info, xl, xu, npt, rhobeg, rhoend, nf, nx, maxfun, params, scaling_changes,
    ...
    if exit_info is not None:
        x, rvec, f, jacmin, nsamples, tolmin = control.model.get_final_results()
        return x, rvec, f, None, nsamples, tolmin, control.nf, control.nx, nruns_so_far + 1, exit_info, diagnostic_info
    ...
    # ------------------------------------------
    current_iter = -1
    last_diag_info_outfile = None
    logging.info("Beginning main loop")
    while True:
        current_iter += 1

        if params("logging.save_diagnostic_info") and diag_info_save_filestem is not None:
            if (current_iter + 1) % diag_info_save_freq == 0:
                diag_info_outfile = diag_info_save_filestem.replace('[nrun]', '%g' % (nruns_so_far+1))
                diag_info_outfile = diag_info_outfile.replace('[niter]', '%g' % (current_iter+1))
                df = diagnostic_info.to_dataframe(with_xk=params("logging.save_xk"), with_rk=params("logging.save_rk"))
                df.to_pickle(diag_info_outfile)
                logging.info("Saved diagnostic info: %s" % diag_info_outfile)
                # Now delete last outfile, if exists
                if last_diag_info_outfile is not None:
                    try:
                        os.remove(last_diag_info_outfile)
                    except:
                        logging.info("Cannot delete old diagnostic info: %s" % last_diag_info_outfile)
                last_diag_info_outfile = diag_info_outfile
        ...
        if finished_growing and params("noise.quit_on_noise_level") and control.all_values_within_noise_level(params):
            if params("restarts.use_restarts") and params("restarts.use_soft_restarts"):
                ...
                exit_info = control.soft_restart(number_of_samples, nruns_so_far, params,
                                     x_in_abs_coords_to_save=None, rvec_to_save=None, nsamples_to_save=None, tol_to_save=None)
                if exit_info is not None:
                    nruns_so_far += 1
                    break  # quit
                current_iter = -1
        ...
        # Required tolerance on residuals
        fopt = control.model.fopt()
        min_pred_for_ftol = 1e-15 * fopt  # machine precision for evaluating fopt + required_f_tol below
        required_f_tol = max(params("tr_radius.eta1_prime") * pred_reduction, min_pred_for_ftol)
        required_tolopt = (sqrt(fopt + required_f_tol) - sqrt(fopt)) / sqrt(control.m())  # required r-tol
        # current_tolopt = 2.0 * sqrt(control.m() * fopt) * control.model.tolopt() + control.m() * control.model.tolopt() ** 2

        if dynamic_accuracy and pred_reduction > 0.0 and \
                control.model.tolopt() > required_tolopt:  # both measured in r-tol
            logging.info("Dynamic accuracy: xk eval tolerance %g above required tolerance %g, re-evaluating" % (control.model.tolopt(), required_tolopt))
            x = control.model.xopt(abs_coordinates=True)
            number_of_samples = max(nsamples(control.delta, control.rho, current_iter, nruns_so_far), 1)
            rvec_list, f_list, num_samples_run, tol, saved_info, exit_info = \
                control.evaluate_objective(x, number_of_samples, params, f_tol=0.99*required_f_tol,
                                           saved_info=control.model.saved_info_opt(), fval=fopt)
            if exit_info is not None:
                if num_samples_run > 0:
                    control.model.save_point(x, np.mean(rvec_list[:num_samples_run, :], axis=0), num_samples_run, tol,
                                             x_in_abs_coords=True)
                nruns_so_far += 1
                break  # quit
            control.model.change_point(control.model.kopt, control.model.xopt(), rvec_list[:num_samples_run, :], tol,
                                       saved_info=saved_info, allow_kopt_update=False)  # expect x w.r.t xbase, not absolute x
            continue  # next loop

        logging.debug("Trust region step is d = " + str(d[:10]))
        ...
        if dnorm < params("general.safety_step_thresh") * control.rho and not finished_growing and params("growing.safety.do_safety_step"):
                ...
                if exit_info is not None:
                    if exit_info.able_to_do_restart() and params("restarts.use_restarts") and params("restarts.use_soft_restarts"):
                        ...
                        exit_info = control.soft_restart(number_of_samples, nruns_so_far, params,
                                                         ...
                                                         nsamples_to_save=None, tol_to_save=None)
                        if exit_info is not None:
                            nruns_so_far += 1
                            break  # quit
                        current_iter = -1
            ...
            if control.rho > rhoend:
                ...
                if params("restarts.use_restarts") and params("restarts.use_soft_restarts"):
                    ...
                    number_of_samples = max(nsamples(control.delta, control.rho, current_iter, nruns_so_far), 1)
                    rvec_list, f_list, num_samples_run, tol, saved_info, exit_info = control.evaluate_objective(x, number_of_samples,
                                                                                               params)
                    
                    if num_samples_run > 0:
                            control.model.save_point(x, np.mean(rvec_list[:num_samples_run, :], axis=0),
                                                     num_samples_run, tol, x_in_abs_coords=True)
            ...
            # Evaluate new point (if dynamic accuracy, to the desired level)
            x = control.model.as_absolute_coordinates(xnew)
            number_of_samples = max(nsamples(control.delta, control.rho, current_iter, nruns_so_far), 1)
            rvec_list, f_list, num_samples_run, tol, saved_info, exit_info = control.evaluate_objective(x, number_of_samples, params,
                                                                                            f_tol=required_f_tol if dynamic_accuracy else None)
            if exit_info is not None:
                if num_samples_run > 0:
                    control.model.save_point(x, np.mean(rvec_list[:num_samples_run, :], axis=0), num_samples_run, tol,
                                             x_in_abs_coords=True)
            ...
            # Decide whether or not we will change kopt
            # If not accepting any decrease, only change kopt when ratio >= eta1
            allow_kopt_update = params("general.accept_any_decrease") or (ratio >= params("tr_radius.eta1"))

            logging.debug("Updating with knew = %i" % knew)
            control.model.change_point(knew, xnew, rvec_list[:num_samples_run, :], tol, saved_info=saved_info,
                                       allow_kopt_update=allow_kopt_update)  # expect x w.r.t xbase, not absolute x
    ...
    return x, rvec, f, jacmin, nsamples, tolmin, control.nf, control.nx, nruns_so_far, exit_info, diagnostic_info


def solve(objfun, x0, args=(), bounds=None, npt=None, rhobeg=None, rhoend=1e-8, dynamic_accuracy=False,
          maxfun=None, nsamples=None, user_params=None,
          objfun_has_noise=False, scaling_within_bounds=False, move_x0_away_from_bounds=True,
          diag_info_save_filestem=None, diag_info_save_freq=1000):
    n = len(x0)
    ...
    # Set parameters
    params = ParameterList(int(n), int(npt), int(maxfun), objfun_has_noise=objfun_has_noise, dynamic_accuracy=dynamic_accuracy)  # make sure int, not np.int
    ...
    if exit_info is None and params("tr_radius.eta1_prime") > 2.0 * params("tr_radius.eta1") + 1e-12:
        exit_info = ExitInformation(EXIT_INPUT_ERROR, "Trust region parameters: eta1_prime must be at most 2*eta1")
    ...
    if exit_info is not None:
        ...
        results = OptimResults(None, None, None, None, None, 0, 0, 0, exit_flag, exit_msg)
        return results

    # Enforce lower & upper bounds on x0
    if move_x0_away_from_bounds:
    ...
    nf = 0
    nx = 0
    xmin, rmin, fmin, jacmin, nsamples_min, tolmin, nf, nx, nruns, exit_info, diagnostic_info = \
        solve_main(objfun, x0, args, xl, xu, npt, rhobeg, rhoend, maxfun, nruns, nf, nx, nsamples, params,
                   ...
                   dynamic_accuracy=dynamic_accuracy,
                   diag_info_save_filestem=diag_info_save_filestem, diag_info_save_freq=diag_info_save_freq)
    ...
    while params("restarts.use_restarts") and not params("restarts.use_soft_restarts") and nf < maxfun and \
        ...
        if params("restarts.hard.use_old_rk"):
            xmin2, rmin2, fmin2, jacmin2, nsamples2, tolmin2, nf, nx, nruns, exit_info, diagnostic_info = \
                solve_main(objfun, xmin, args, xl, xu, npt, rhobeg, rhoend, maxfun, nruns, nf, nx, nsamples, params,
                           ...
                           dynamic_accuracy=dynamic_accuracy, r0_tol_old=tolmin,
                           diag_info_save_filestem=diag_info_save_filestem, diag_info_save_freq=diag_info_save_freq)
        else:
            xmin2, rmin2, fmin2, jacmin2, nsamples2, tolmin2, nf, nx, nruns, exit_info, diagnostic_info = \
                solve_main(objfun, xmin, args, xl, xu, npt, rhobeg, rhoend, maxfun, nruns, nf, nx, nsamples, params,
                           diagnostic_info, scaling_changes, dynamic_accuracy=dynamic_accuracy,
                           diag_info_save_filestem=diag_info_save_filestem, diag_info_save_freq=diag_info_save_freq)

        if fmin2 < fmin or np.isnan(fmin):
            logging.info("Successful run with new f = %s compared to old f = %s" % (fmin2, fmin))
            last_successful_run = nruns
            (xmin, rmin, fmin, nsamples_min, tolmin) = (xmin2, rmin2, fmin2, nsamples2, tolmin2)
    ...
    results = OptimResults(remove_scaling(xmin, scaling_changes), rmin, fmin, jacmin, tolmin, nf, nx, nruns, exit_flag, exit_msg)
    if params("logging.save_diagnostic_info"):
        df = diagnostic_info.to_dataframe(with_xk=params("logging.save_xk"), with_rk=params("logging.save_rk"))
