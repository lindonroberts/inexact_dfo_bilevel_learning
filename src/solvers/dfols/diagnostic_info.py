...
class DiagnosticInfo(object):
    def __init__(self):
        ...
        self.data["xk"] = []
        self.data["rk"] = []
        self.data["rk_tol"] = []  # max error on r(xk)
        self.data["fk_tol"] = []  # max error on f(xk)
        self.data["fk"] = []
        self.data["eval_work_done"] = []  # total work done so far (as measured by dynamic accuracy objfun)
    ...
    def save_info_from_control(self, control, nruns, iter_this_run, save_poisedness=True):
        ...
        x, rvec, f, jac, nsamples, rtol = control.model.get_final_results()
        self.data["xk"].append(remove_scaling(x, control.scaling_changes))
        self.data["rk"].append(rvec)
        self.data["fk"].append(f)
        self.data["rk_tol"].append(rtol)
        self.data["fk_tol"].append(2.0 * sqrt(control.m() * f) * rtol + control.m() * rtol ** 2)
        self.data["eval_work_done"].append(control.work_done)
