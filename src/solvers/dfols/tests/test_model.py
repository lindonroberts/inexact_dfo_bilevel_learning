...
class TestAddValues(unittest.TestCase):
    def runTest(self):
        ...
        xl = -1e20 * np.ones((n,))
        xu = 1e20 * np.ones((n,))
        model = Model(npt, x0, rosenbrock(x0), xl, xu, 1, 0.0, None)
        ...
        x1 = np.array([1.0, 0.9])
        rvec = rosenbrock(x1)
        model.change_point(1, x1 - model.xbase, rvec, 0.0, allow_kopt_update=True)
        ...
        x2 = np.array([2.0, 0.9])
        rvec = rosenbrock(x2)
        model.change_point(2, x2 - model.xbase, rvec, 0.0, allow_kopt_update=True)
        ...
        x3 = np.array([1.0, 1.0])
        rvec = rosenbrock(x3)
        model.change_point(0, x3 - model.xbase, rvec, 0.0, allow_kopt_update=False)  # full: overwrite x0
...
class TestBasicManipulation(unittest.TestCase):
    def runTest(self):
        ...
        x1 = np.array([1.0, 0.9])
        model.change_point(1, x1 - model.xbase, rosenbrock(x1), 0.0)
        ...
        x2 = np.array([2.0, 0.9])
        model.change_point(2, x2 - model.xbase, rosenbrock(x2), 0.0)
        ...
        for i in range(50):
            model.add_new_sample(0, np.array([0.0, 0.0]), do_kopt_update=True)
        ...
        model.change_point(0, x0 - model.xbase, rosenbrock(x0), 0.0)  # revert after resampling
        model.change_point(1, x1 - model.xbase, rosenbrock(x1), 0.0)  # revert after resampling
        x, rvec, f, jacmin, nsamples, tolmin = model.get_final_results()
        ...
        model.save_point(x0, rosenbrock(x0), 1, 0.0, x_in_abs_coords=True)
        ...
        model.save_point(x2 - model.xbase, np.array([0.0, 0.0]), 2, 0.0, x_in_abs_coords=False)
        ...
        model.save_point(x0, rosenbrock(x0), 3, 0.0, x_in_abs_coords=True)  # try to re-save a worse value
...
class TestAveraging(unittest.TestCase):
    def runTest(self):
        ...
        for i in range(10):
            model.add_new_sample(2, np.array([5.0, 5.0]), do_kopt_update=True)
...
class TestMinObjValue(unittest.TestCase):
    def runTest(self):
        ...
        model = Model(npt, x0, rosenbrock(x0), xl, xu, 1, 0.0, None, rel_tol=1e-2)
        self.assertAlmostEqual(model.min_objective_value(), 1e-2 * sumsq(rosenbrock(x0)), 'Wrong min obj value 2')
        model = Model(npt, x0, rosenbrock(x0), xl, xu, 1, 0.0, None, abs_tol=1.0)
        self.assertAlmostEqual(model.min_objective_value(), 1.0, 'Wrong min obj value 3')
        model = Model(npt, x0, rosenbrock(x0), xl, xu, 1, 0.0, None, abs_tol=1.0, rel_tol=1e-2)
...
class TestInterpMatrixSVD(unittest.TestCase):
    def runTest(self):
        ...
        self.assertTrue(array_compare(hess, 2.0*np.dot(J.T, J)), 'Bad Hessian')
...
class TestAddPoint(unittest.TestCase):
    def runTest(self):
        ...
        model.add_new_point(x3 - model.xbase, rosenbrock(x3), 0.0)
        ...
        model.add_new_point(x4 - model.xbase, rosenbrock(x4), 0.0)
...
class TestRegression(unittest.TestCase):
    def runTest(self):
        ...
        xl = -1e2 * np.ones((n,))
        xu = 1e2 * np.ones((n,))
        model = Model(npt, A[0,:], np.array([b[0]]), xl, xu, 1, 0.0, None)

        for i in range(1,npt):
            xi = A[i,:]
            model.change_point(i, xi - model.xbase, np.array([b[i]]), 0.0)
...
class TestUnderdetermined(unittest.TestCase):
    def runTest(self):
        ...
        xl = -1e2 * np.ones((n,))
        xu = 1e2 * np.ones((n,))
        # model = Model(n+1, x0, np.array([0.0]), xl, xu, 1, 0.0, None)
        model = Model(n+1, A[0, :], np.array([b[0]]), xl, xu, 1, 0.0, None)
